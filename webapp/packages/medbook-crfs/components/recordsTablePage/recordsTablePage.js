...
Template.recordsTablePage.events({
  ...
  'click .starA': function (event, template){
    ...
    CRFs.update({_id: this._id}, {$set: {stars: 1}});
  },
  'click .starB': function (event, template){
    event.stopPropagation();
    CRFs.update({_id: this._id}, {$set: {stars: 2}});
  },
  'click .starC': function (event, template){
    event.stopPropagation();
    CRFs.update({_id: this._id}, {$set: {stars: 3}});
  },
  'click .starD': function (event, template){
    event.stopPropagation();
    CRFs.update({_id: this._id}, {$set: {stars: 4}});
  },
  'click .starE': function (event, template){
    event.stopPropagation();
    CRFs.update({_id: this._id}, {$set: {stars: 5}});
  },
  'click .checkbox': function (event, template){
    event.stopPropagation();
    if (this.checked){
      CRFs.update({_id: this._id}, {$set: {checked: false}});
    } else {
      CRFs.update({_id: this._id}, {$set: {checked: true}});
    }
  },
  'click .flag': function (event, template){
    event.stopPropagation();
    if (this.flagged){
      CRFs.update({_id: this._id}, {$set: {flagged: false}});
    } else {
      CRFs.update({_id: this._id}, {$set: {flagged: true}});
    }
  },
